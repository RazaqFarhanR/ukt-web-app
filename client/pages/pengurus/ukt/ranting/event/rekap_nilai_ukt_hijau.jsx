...
const rekap_nilai_ukt_hijau = () => {
    ...
    useEffect(() => {
        ...
        isLogged()
        const pengurusData = JSON.parse(localStorage.getItem('pengurus'));
        if (pengurusData) {
            setPengurusRole(pengurusData.id_role);
        }
    }, [])
    ...
    const getDataEventSelect = async () => {
        ...
        const token = localStorage.getItem('token')
        await
            axios.get(BASE_URL + `event/select/tipe/UKT HIJAU/all`, { headers: { Authorization: `Bearer ${token}` } })
                .then(res => {
                    ...
                    <div className="h-full bg-darkBlue p-6">
                        ...
                        <div className="flex justify-between items-center text-white mb-7">
                            ...
                            <div className="flex flex-col md:flex-row gap-3 w-full items-start md:items-center justify-end">
                                ...
                                {/* actions dropdown */}
                                {(pengurusRole === 'pengurus cabang' || pengurusRole === 'super pengurus') && (
