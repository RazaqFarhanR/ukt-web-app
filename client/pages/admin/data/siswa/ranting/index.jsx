...
const siswa = () => {
    ...
    const getDatasiswa = () => {
        ...
        axios.get(BASE_URL + `siswa/count/siswa/${idTipe}`, { headers: { Authorization: `Bearer ${token}` } })
            .then(res => {
    ...
    const goToRantingSiswa = (item) => {
        router.push({
            pathname: `./ranting/event`,
            query: { tipe: idTipe, ranting: item.id_ranting }
        });
    }
                                ...
                                <button onClick={() => goToRantingSiswa(item)} key={index + 1} href={'./' + item.id_ranting} className="bg-navy hover:bg-gradient-to-r from-[#16D4FC] to-[#9A4BE9] rounded-md p-0.5">
                                    ...
                                    <div className="bg-navy p-5 rounded-md space-y-5">
                                        ...
                                        <div className='flex justify-between'>
                                            <h1 className='text-green text-lg'>{item.id_ranting}</h1>
                                        </div>
