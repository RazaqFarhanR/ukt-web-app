...
const belati = (props) => {
    const [listbelati, setListbelati] = useState([])
    const [databelati, setDatabelati] = useState([])
    ...
    const getDataListbelati = async () => {
        const token = localStorage.getItem('token')
        try {
            const res = await axios.get(BASE_URL + `belati_detail/list`, {
                headers:
                    { Authorization: `Bearer ${token}` }
            })
            setListbelati(res.data.data)
        } catch (err) {
            console.log(err.message);
        }
    }

    const getDatabelati = async () => {
        const cacheKey = `${props.data?.ranting}-${page}`;
        if (cache.current[cacheKey]) {
            const cached = cache.current[cacheKey];
            setDatabelati(cached.data);
                    ...
                    <table className='w-max'>
                        <thead className='sticky top-0 bg-black'>
                            <tr className='text-white'>
                                ...
                                {listbelati?.slice(0, 1).map((item, index) => (
                                    <ThComponent items={listbelati} key={index + 1} />
                                ))}
                            </tr>

                        </thead>
                        <tbody>
                            {databelati?.map((item, index) => (
                                <>
                                    <tr className='text-green text-center' key={item.id_belati_detail}>
