...
const ujian = () => {
    ...
    // Effect to flush the sync queue whenever online or queue changes
    useEffect(() => {
        ...
        const interval = setInterval(flushQueue, 3000); // Retry every 3 seconds
    ...
    const handleSave = () => {
        ...
        // Optimized backend call that scores everything at once
    ...
    const Logout = () => {
        try {
            ...
            localStorage.clear()
            router.push(`/siswa/${tipe}/${ranting}/${encryptId(JSON.stringify({id:dataEvent.id,name:dataEvent.name}))}`)
        } catch (e) {
            localStorage.clear()
            router.push('/siswa')
        }
        ...
        <div className={`font-lato bg-darkBlue w-full min-h-screen h-auto flex flex-col items-center`}>
            ...
            <div className='w-[90%] my-4'>
                ...
                {soal ? (
                    ...
                    <div className='w-full bg-navy rounded-lg lg:my-2 lg:p-3 p-2 flex justify-center items-center h-48'>
                        <p className='text-white lg:text-2xl text-lg font-lato text-center'>Data soal tidak ditemukan atau sesi ujian tidak valid. Harap lapor ke admin.</p>
                    </div>
                )}
